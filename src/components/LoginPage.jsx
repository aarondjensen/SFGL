...
const styles = `
  ...
  .sfgl-login-header {
    text-align: center;
    margin-bottom: 36px;
    animation: sfglFadeUp 0.4s 0.1s cubic-bezier(0.22,1,0.36,1) both;
  }
  .sfgl-logo-wrap {
  ...
  .sfgl-logo-svg { width: 32px; height: 32px; }
  .sfgl-login-title {
    font-family: 'Playfair Display', serif;
    font-size: 26px; font-weight: 700;
    color: #f0e8d2; letter-spacing: -0.2px; line-height: 1.2;
  }
  .sfgl-login-title span { color: #c9a84c; }
  .sfgl-login-subtitle {
    margin-top: 6px; font-size: 11px; font-weight: 400;
    color: rgba(240,232,210,0.4); letter-spacing: 2px; text-transform: uppercase;
  }
  ...
  .sfgl-divider-gem { color: rgba(201,168,76,0.35); font-size: 9px; letter-spacing: 2px; }

  /* ── Fields ── */
  .sfgl-field { margin-bottom: 18px; animation: sfglFadeUp 0.4s cubic-bezier(0.22,1,0.36,1) both; }
  .sfgl-field:nth-of-type(1) { animation-delay: 0.15s; }
  .sfgl-field:nth-of-type(2) { animation-delay: 0.2s; }

  .sfgl-label {
    display: block; font-size: 10px; font-weight: 500;
    color: rgba(240,232,210,0.45); letter-spacing: 1.5px;
  ...
  .sfgl-toggle {
    ...
    color: rgba(240,232,210,0.28); padding: 4px;
    display: flex; align-items: center; transition: color 0.2s;
  }
  .sfgl-toggle:hover { color: rgba(240,232,210,0.6); }

  .sfgl-field-error {
    margin-top: 5px; font-size: 11.5px; color: #e07070;
  ...
  .sfgl-checkbox:checked::after {
    ...
    border: 2px solid #f0e8d2; border-top: none; border-left: none;
  ...
  .sfgl-btn:hover {
    box-shadow: 0 4px 20px rgba(58,125,82,0.4), 0 0 0 1px rgba(201,168,76,0.28);
  ...
  .sfgl-spinner {
    ...
    border: 2px solid rgba(240,232,210,0.25);
    border-top-color: #f0e8d2; border-radius: 50%;
...
const GolfFlag = () => (
  <svg viewBox="0 0 32 32" fill="none" className="sfgl-logo-svg">
    ...
    <line x1="10" y1="23" x2="10" y2="6"
      stroke="rgba(240,232,210,0.9)" strokeWidth="1.8" strokeLinecap="round"/>
...
//
// Props:
//   onLogin({ team, sessionToken }) — called after successful login so App.jsx
//                                     can update loggedInUser state
// ─────────────────────────────────────────────────────────────────────────────
export default function LoginPage({ onLogin }) {
  ...
  const handleSubmit = async (evt) => {
    ...
    try {
      // Calls Supabase directly via managerAuthApi:
      //   1. Finds team row by manager_email
      //   2. Checks manager_password_hash
      //   3. Creates a manager_sessions row (60-day expiry)
      //   4. Saves token to localStorage automatically
      //   Returns { team, sessionToken }
      const result = await managerAuthApi.login(email, password);
    ...
    <>
      ...
      <div className="sfgl-login-root">
        <div className="sfgl-login-card">
          ...
          {/* Gold divider */}
          <div className="sfgl-divider">
            <span className="sfgl-divider-gem">✦</span>
          </div>
          ...
          <form onSubmit={handleSubmit} noValidate>
            ...
            {serverError && (
              <div className="sfgl-server-error">⚠ {serverError}</div>
            )}

            {/* Email */}
            <div className="sfgl-field">
              <label htmlFor="sfgl-email" className="sfgl-label">Email Address</label>
              <div className="sfgl-field-wrap">
                <input
                  id="sfgl-email"
                  type="email"
                  className="sfgl-input"
              ...
              {errors.email && (
                <p className="sfgl-field-error">⚠ {errors.email}</p>
              )}
            </div>
            ...
            <div className="sfgl-field">
              ...
              <div className="sfgl-field-wrap">
                <input
                  ...
                  className="sfgl-input with-toggle"
                  placeholder="••••••••"
                  value={password}
          ...
          <div className="sfgl-footer">
            <p>SFGL Season 2026 · Managers only</p>
          </div>
